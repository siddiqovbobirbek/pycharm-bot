...
choose_language.insert(KeyboardButton('🇺🇿 O\'zbekcha')).insert(KeyboardButton('🇷🇺 Русский')).insert(
...
main_en = ReplyKeyboardMarkup(resize_keyboard=True)
main_en.insert(KeyboardButton(text="📝 Menu")).row(KeyboardButton(text="📖 My orders"), KeyboardButton(text="📥 Basket"),
...
def product_or_subcategory(category, language, product=None):
    ...
    if 'subcategory' in data:
        ...
        if language == 'ru':
            ...
            # add to basket
            button.add(InlineKeyboardButton(text="📥 Add to basket",
                                            callback_data=basket_callback.new(action='add', product=data[0]['id'])))
        else:
            # savatga qo'shish
...
def settings(language):
    ...
    button.row(InlineKeyboardButton(text="🇺🇿 O'zbekcha"), InlineKeyboardButton(text="🇷🇺 Русский"),
               InlineKeyboardButton(text="en English"))
    if language == 'ru':
        # return to main menu
...
def product_button(data, language):
    ...
    if language == 'ru':
        ...
        button.add(InlineKeyboardButton(text="📥 Add to basket", callback_data=basket_callback.new(action='add', count=1,
                                                                                                  product=product[
                                                                                                      'id'])))
    else:
        button.add(InlineKeyboardButton(text="📥 Savatga qo'shish",
